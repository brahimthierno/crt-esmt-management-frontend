...
// NOUVELLE VERSION POUR REGLER LE PROBLEME AVEC LE DEPLOIEMENT POUR DEMANDES ET STATS
...
//         // Statistiques
//         statsService.getAllStats().then(res => {
...
//       console.log('- Emprunts:', empruntsData?.length || 0);
//       console.log('- Demandes:', demandesData?.length || 0);
//       console.log('- Statistiques chargées:', Object.keys(statsData).length > 0 ? 'Oui' : 'Non');
...
//                   <StatistiquesView 
//                     currentUser={currentUser}
//                     statistiques={statistiques.detaillees} // Passer directement les stats détaillées
//                     onExportPDF={() => statsService.exportPDF(statistiques.detaillees, currentUser)}
//                   />
//                 )}
...
// NOUVELLE VERSION POUR LES COMMENTAIRES
...
const App = () => {
  ...
  // ✅ NOUVEAU : Gestion des commentaires
  const addComment = async (interventionId, texte) => {
    try {
      console.log('🔄 Ajout commentaire pour intervention:', interventionId);
      const response = await commentService.addComment(interventionId, texte);
      
      if (response.success) {
        // Mettre à jour l'intervention avec le nouveau commentaire
        setInterventions(interventions.map(i => 
          i._id === interventionId ? response.data : i
        ));
  ...
  const deleteComment = async (interventionId, commentId) => {
    try {
      console.log('🔄 Suppression commentaire:', commentId, 'de l\'intervention:', interventionId);
      const response = await commentService.deleteComment(interventionId, commentId);
      
      if (response.success) {
        // Mettre à jour l'intervention après suppression
        setInterventions(interventions.map(i => 
          i._id === interventionId ? response.data : i
        ));
      ...
      return { 
        success: false, 
        message: err.response?.data?.message || 'Erreur lors de la suppression du commentaire' 
      };
    }
  };

  const updateComment = async (interventionId, commentId, texte) => {
    try {
      console.log('🔄 Modification commentaire:', commentId);
      const response = await commentService.updateComment(interventionId, commentId, texte);
      
      if (response.success) {
        // Mettre à jour l'intervention après modification
        setInterventions(interventions.map(i => 
          i._id === interventionId ? response.data : i
        ));
      ...
      return { 
        success: false, 
        message: err.response?.data?.message || 'Erreur lors de la modification du commentaire' 
      };
    }
  };
            ...
            <div className="container mx-auto px-4 py-6 flex gap-6">
              ...
              <main className="flex-1">
                ...
                {activeView === 'statistiques' && (
                  <StatistiquesView 
                    currentUser={currentUser}
                    statistiques={statistiques.detaillees}
                    onExportPDF={() => statsService.exportPDF(statistiques.detaillees, currentUser)}
                  />
                )}
