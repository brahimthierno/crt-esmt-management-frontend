...
import { AlertCircle, CheckCircle, Phone, Mail, Building, MapPin, Wrench } from 'lucide-react';
import { getConfigDemande, createDemande } from '../../services/demandeService'; // ✅ Import du service
...
const DemandeAnonymePage = () => {
  ...
  const [config, setConfig] = useState({
    equipements: { categories: { INFORMATIQUE: [], ELECTRIQUE: [] }, allEquipements: [] },
    batiments: {}
  });
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        const data = await getConfigDemande();
        setConfig(data);
      } catch (error) {
  ...
  // ✅ Gérer le changement de bâtiment
  const handleBatimentChange = (e) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      batiment,
      lieu: '' // Réinitialiser le lieu quand le bâtiment change
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        setNumeroReference(response.numeroReference);
        setSubmitted(true);
        setFormData({
          batiment: '',
          lieu: '',
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                >
                  ...
                  {Object.keys(config.batiments).map((batiment) => (
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                >
                  ...
                  {formData.batiment && config.batiments[formData.batiment]?.map((site) => (
              ...
              {!formData.batiment && (
                <p className="text-xs text-gray-500 mt-1">Veuillez d'abord sélectionner un bâtiment</p>
              )}
            </div>

            {/* ✅ Équipement (liste déroulante) */}
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Équipement concerné *
              </label>
              <div className="flex items-center">
                ...
                >
                  ...
                  <optgroup label="Équipements Informatiques">
                    {config.equipements.categories.INFORMATIQUE.map((equipement) => (
                  ...
                  <optgroup label="Équipements Électriques">
                    {config.equipements.categories.ELECTRIQUE.map((equipement) => (
                      <option key={equipement} value={equipement}>
                        {formatEquipementName(equipement)}
                      </option>
                    ))}
