...
// NOUVELLE VERSION POUR REGLER LE MENU UTILISATEUR DANS LE HEADER (POUR QU'IL S'ACTUALISE INSTANTANNEMENT APRES MODIFICATION DU PROFIL)
...
const Header = ({ setActiveView }) => { // ✅ RETIREZ currentUser ET onLogout DES PROPS
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);
  const menuRef = useRef(null);
  const navigate = useNavigate();
  ...
  // ✅ UTILISEZ LE CONTEXTE AUTH POUR OBTENIR LES DONNÉES UTILISATEUR
  ...
  const handleLogout = () => {
    setShowUserMenu(false);
    logout(); // ✅ UTILISEZ LA FONCTION DU CONTEXTE
  };
