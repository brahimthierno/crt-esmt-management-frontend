...
import React, { useState } from 'react';
import { MessageSquare, Send, Trash2, User, Clock, Edit2, X } from 'lucide-react';
...
const CommentSection = ({ 
  ...
  const handleUpdate = async (commentId) => {
    ...
    setIsSubmitting(true);
    try {
      await onUpdateComment(intervention._id, commentId, editedText.trim());
      setEditingCommentId(null);
      setEditedText('');
    } catch (error) {
                      ...
                      {isEditing ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            className="w-full px-3 py-2 bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 resize-none text-gray-900 dark:text-white text-sm"
                            rows="3"
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg transition-all duration-200 text-sm font-medium"
                            >
                              {isSubmitting ? (
                                <>
                            ...
                            <button
                              ...
                              disabled={isSubmitting}
                              className="flex items-center gap-1 px-3 py-1.5 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-300 rounded-lg transition-all duration-200 text-sm font-medium"
                            >
                              <X size={14} />
                              Annuler
                            </button>
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-1 mt-2 text-xs text-gray-500 dark:text-gray-400">
                            <Clock size={12} />
                            <span>{formatDate(comment.date)}</span>
                            {comment.modifie && comment.dateModification && (
                              <>
                                <span className="mx-1">•</span>
                                <span>Modifié {formatDate(comment.dateModification)}</span>
                              </>
                            )}
                          </div>
                        </>
                    ...
                    <div className="flex items-center gap-1">
                      {canEdit && (
                        <button
                          ...
                          className="p-1.5 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all duration-200"
                          title="Modifier"
                          disabled={loading}
                        >
                          <Edit2 size={14} />
